...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        while (!init_ack)
        {
            ...
            {
                if ((nbytes = recv(sockfd, (void *)&recv_packet, recv_packet.getSegmentSize(), 0)) == -1)
                {
            ...
            {
                if (sendto(sockfd, (void *)&synack_packet, synack_packet.getSegmentSize(), 0, (struct sockaddr *)&client_addr, client_addr_length) == -1)
                {
                    perror("sendto() error in server while sending SYN-ACK/SYN");
                }
            }
